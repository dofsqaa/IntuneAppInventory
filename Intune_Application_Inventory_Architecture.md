# Intune Application Inventory & Security Group Management Architecture

## System Architecture Overview

This architecture provides a scalable local inventory database, a PowerShell GUI management console, and an automated background synchronization service. Instead of relying on Entra ID dynamic groups to perform application detection, this solution uses Microsoft Graph API batch collection endpoints, ingests data into a local relational SQLite database, and reconciles static Entra ID Security Groups for dynamic Intune targeting.
...
```
    ...
    /deviceManagement/managedDevices   /deviceManagement/detectedApps
               ...
               └───────────────┬───────────────┘
                               ▼
               Sync Engine (Pagination + Upsert)
                               │
                               ▼
                        SQLite Database
                   (Devices, Apps, DeviceApps)
                               │
            ┌──────────────────┴──────────────────┐
            ▼                                     ▼
   Interactive GUI Mode                  Scheduled Sync Mode
  (WPF / Search / Target Groups)     (Entra Group Reconciliation)
```
...
The system utilizes five relational SQLite tables to ensure scalability, historical tracking, and efficient version comparison queries.
...
├──────────────────────────┤       ├──────────────────────────┤
│ DeviceId (PK)            │       │ AppId (PK)               │
│ AzureADDeviceId          │       │ DisplayName              │
│ DeviceName               │       │ Version                  │
│ UserPrincipalName        │       │ Publisher                │
│ ManagementAgent          │       │ Platform                 │
│ OwnerType                │       │ SizeInBytes              │
│ ComplianceState          │       │ DeviceCount              │
...
┌──────────────────────────┐       ┌──────────────────────────┐
│      ManagedGroups       │       │       SyncHistory        │
├──────────────────────────┤       ├──────────────────────────┤
│ GroupId (PK)             │       │ SyncId (PK)              │
│ GroupName                │       │ Started                  │
│ AppId                    │       │ Completed                │
│ AppName                  │       │ DeviceCount              │
│ AppVersion               │       │ ApplicationCount         │
│ ComparisonOperator       │       │ RelationshipCount        │
│ Enabled                  │       │ Status                   │
...
## 2. Query Reference & Analysis

### Exact Application & Version Lookup
```sql
SELECT
    d.DeviceName,
    d.UserPrincipalName,
...
### Application Breakdown Across Fleet
```sql
SELECT 
    a.DisplayName,
    da.DetectedVersion,
...
JOIN Applications a ON a.AppId = da.AppId
WHERE a.DisplayName = 'Microsoft Edge'
GROUP BY da.DetectedVersion
ORDER BY da.DetectedVersion DESC;
```

#### Sample Query Output
| DisplayName | DetectedVersion | DeviceCount | Status |
| :--- | :--- | :--- | :--- |
| Microsoft Edge | `151.0.4129.59` | 1,240 | Compliant |
| Microsoft Edge | `150.0.4099.10` | 183 | Upgrade Required |
| Microsoft Edge | `149.0.3950.4` | 42 | Upgrade Required |
| Microsoft Edge | `Unknown` | 11 | Investigation Required |
...
├── IntuneInventory.ps1      # Main Application (GUI & Sync Engine)
├── IntuneInventory.db       # SQLite Database
├── config.json              # Configuration & Authentication Settings
├── System.Data.SQLite.dll   # SQLite Core Driver
│
├── x64│   └── SQLite.Interop.dll   # Native x64 Interop Assembly
│
└── Logs    ├── Sync.log             # Operational Execution Log
    └── Error.log            # Failure Tracking Log
```

---

## 4. Complete Script Implementation (`IntuneInventory.ps1`)
...
### Application Permissions (Entra ID App Registration)

Grant the following **Application** permissions to the registered Entra Application:

| API / Permission Name | Permission Type | Purpose |
| :--- | :--- | :--- |
| `DeviceManagementManagedDevices.Read.All` | Application | Read Intune inventory & device identifiers |
| `DeviceManagementApps.Read.All` | Application | Read globally detected software list |
| `Group.ReadWrite.All` | Application | Create and update target Entra security groups |
| `Device.Read.All` | Application | Resolve Azure AD Device Identifiers to Object IDs |

### Configuration File Template (`config.json`)

Save in `C:\ProgramData\IntuneInventory\config.json`:
